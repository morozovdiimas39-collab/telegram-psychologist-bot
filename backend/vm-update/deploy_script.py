#!/usr/bin/env python3
"""
Скрипт деплоя для установки на VM.
Устанавливается на сервер и принимает webhook запросы.
"""
...
class DeployHandler(BaseHTTPRequestHandler):
    """HTTP обработчик для webhook деплоя"""
...
def deploy_project(github_url: str, project_name: str, domain: str, secrets: List[Dict]) -> Dict:
    """
    Деплой проекта на VM:
    1. Клонирование репозитория
    2. Сборка фронтенда (Vite)
    3. Установка PostgreSQL
    4. Настройка облачных функций
    5. Миграция БД
    6. Настройка Nginx
    """
    ...
    pg_password = "postgres"  # Задаётся при создании VM через cloud-init
    ...
    if backend_dir.exists():
        ...
        env_vars = {
            'DATABASE_URL': f'postgresql://postgres:{pg_password}@localhost/{db_name}'
        }
        
        # Добавляем секреты из запроса
        ...
        # Установка зависимостей для Python функций
        for func_dir in backend_dir.iterdir():
            if func_dir.is_dir() and (func_dir / 'requirements.txt').exists():
                ...
                subprocess.run([
                    'pip3', 'install', '-r', str(func_dir / 'requirements.txt'),
                    '--target', str(func_dir / 'packages')
                ], check=True)
        
        # Запуск API сервера для функций (простой Flask)
...
"""
    ...
    logs.append(f"🎉 Деплой завершён! Сайт доступен по адресу: http://{domain}")
...
def setup_functions_api(backend_dir: Path, env_vars: Dict, logs: List[str]):
    """
    Создаёт Flask API сервер для облачных функций.
    Каждая функция становится эндпоинтом /api/{function_name}
    """
...
def call_function(function_name):
    ...
    # Подготовка event объекта (совместимый с Cloud Functions)
    event = {
...
'''
    ...
    # Создаём systemd сервис для API
