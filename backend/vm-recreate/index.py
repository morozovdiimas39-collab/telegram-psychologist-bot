...
def handler(event: dict, context) -> dict:
    """Удалить и пересоздать VM с правильным SSH ключом"""
    ...
    try:
        ...
        # Удаляем старую VM если найдена
        if vm_id:
            logs.append(f"🗑️ Удаляю старую VM: {vm_id}...")
            ...
            if delete_resp.status_code in [200, 202]:
                ...
                logs.append("⏳ Жду 30 секунд...")
                import time
                time.sleep(30)
            else:
                logs.append(f"⚠️ Ошибка удаления: {delete_resp.text}")
        else:
            logs.append("ℹ️ VM не найдена, создаю новую")
        ...
        # Cloud-init с SSH ключом и скриптом
...
"""
        ...
        # Создаем VM
        logs.append("🚀 Создаю новую VM с SSH ключом...")
        ...
        logs.append("⏳ Жду 60 секунд для инициализации...")
        
        import time
        time.sleep(60)
        
        # Получаем IP адрес
        logs.append("🔍 Получаю IP адрес...")
        instances_resp = requests.get(
            f"https://compute.api.cloud.yandex.net/compute/v1/instances?folderId={folder_id}",
            headers=headers,
            timeout=10
        ...
        if not vm_ip:
            logs.append("⚠️ IP не найден, жди ещё минуту")
        else:
            logs.append(f"✅ IP адрес: {vm_ip}")
        
        webhook_url = f"http://{vm_ip}:9000/deploy" if vm_ip else "Ожидание IP..."
        
        logs.append("")
        logs.append("🎉 VM пересоздана с SSH ключом!")
        logs.append("")
        logs.append("📋 Сохрани эти секреты:")
        logs.append(f"  VM_IP_ADDRESS: {vm_ip}")
        logs.append(f"  VM_WEBHOOK_URL: {webhook_url}")
        logs.append("  VM_SSH_KEY: (приватный ключ ниже)")
        logs.append("")
        logs.append("⏳ Systemd сервис запустится через 2-3 минуты")
        logs.append(f"💡 Проверь через 3 минуты: {webhook_url}")
