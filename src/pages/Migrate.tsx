...
export default function Migrate() {
  ...
  const handleMigrateFromGitHub = async () => {
    ...
    if (!githubToken.trim()) {
      toast.error('Укажи GitHub токен (нужен для доступа к репозиторию)');
      return;
    }
        ...
        <Card className="bg-white/10 backdrop-blur border-white/20">
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <Input
                type="password"
                placeholder="ghp_... или токен с правами repo"
                value={githubToken}
                ...
                autoComplete="off"
              />
              <p className="text-xs text-slate-400 mt-1">Нужен для доступа к репо (Settings → Developer settings → Personal access tokens).</p>
            </div>
            <Button
              onClick={handleMigrateFromGitHub}
              disabled={!githubRepo.trim() || !githubToken.trim() || loading}
